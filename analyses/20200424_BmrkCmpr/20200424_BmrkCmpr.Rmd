...
SEG_uniq <- read.table("/Volumes/HHD/Downloads/16C_26psu_3_S15_L001_R1_001.SEGuniq.vPE.5x.txt", sep = "\t", header = F)

colnames(SEG_uniq) <- c("Chromosome", "Start", "End", "Perc_meth", "methCpG_reads", "totalCpG_reads")

#plot number of 5x CpG loci x number of reads
jpeg("figs/SEGuniq_vPE_numReadsXnumCpGloci.jpg", width = 4, height = 3, units = "in", res = 300)
ggplot(SEG_uniq) + geom_histogram(aes(totalCpG_reads),color = "black", fill = "lightblue") + theme_bw() + ylab("number of 5x CpG loci") + xlab("number of reads")
```


read in data
```{r}
PE_stats <- read.table("PE_multiqc_general_stats.txt", sep = "\t", header = T, stringsAsFactors = F)

SEG_stats <- read.table("SEG_multiqc_general_stats.txt", sep = "\t", header = T, stringsAsFactors = F)

SEL_stats <- read.table("SEL_multiqc_general_stats.txt", sep = "\t", header = T, stringsAsFactors = F)

total_reads <- read.table("total_reads.txt", sep = "\t", header = T)
```

add column to describe alignment parameter used
```{r}
PE_stats$method <- "PE"
SEG_stats$method <- "SEG"
SEL_stats$method <- "SEL"
...
combine stat tables
```{r}
stats <- rbind(PE_stats, SEG_stats, SEL_stats)

```

reformat table
```{r}
stacked_stats <- tidyr::gather(stats, "stat", "count",2:15)

```

remove NAs
```{r}
stacked_stats <- stacked_stats[which(!is.na(stacked_stats$count)),]
```

clean up stat names
```{r}
stacked_stats$stat <- gsub("Bismark_mqc.generalstats.","", stacked_stats$stat)
stacked_stats$stat <- gsub("Cutadapt_mqc.generalstats.","", stacked_stats$stat)
stacked_stats$stat <- gsub("FastQC_mqc.generalstats.","", stacked_stats$stat)

#remove val part from sample name
stacked_stats$Sample <- gsub("_val.*","", stacked_stats$Sample)
```

plot
```{r}
ggplot(stacked_stats[grep("total_sequence", stacked_stats$stat),]) + geom_point(aes(x = method,y=count, color = method, group= Sample), position = "jitter") + facet_wrap(~Sample, scale = "free")


ggplot(stacked_stats) + geom_boxplot(aes(x = method,y=count, fill = method,group = method)) + facet_wrap(~stat, scale = "free")


ggplot(stacked_stats) + geom_point(aes(x = method,y=count, color = method,group = method), position = "jitter") + facet_wrap(~stat, scale = "free")
```