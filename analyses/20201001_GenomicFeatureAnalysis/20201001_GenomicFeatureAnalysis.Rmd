...
ggplot(DMR_bkgd_features[grep("first_exon|last_exon", DMR_bkgd_features$feature_simp),],aes(x = comparison, y = ..count.., fill = factor(feature_simp), group = feature_simp)) + geom_bar(position = "fill", color = "black") + scale_y_continuous(labels=scales::percent) + ylab("% of total regions") + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 60, hjust = 1), axis.title = element_text(size = 12, face = "bold")) + scale_fill_manual("Feature",values = RColorBrewer::brewer.pal(7,"BuGn")) + ggtitle("first and last exons percentages")

ggplot(DMR_bkgd_features_totals[grep("exon",DMR_bkgd_features_totals$feature_simp),],aes(x = feature_simp, y = count, fill = feature_simp)) + geom_bar(stat = "identity", color = "black") + ylab("count") + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 60, hjust = 1), axis.title = element_text(size = 12, face = "bold")) + ggtitle("first and last exon counts") + facet_wrap(~comparison, scale = "free") + scale_fill_manual("Feature",values = RColorBrewer::brewer.pal(7,"BuGn"))


ggplot(DMR_bkgd_features[grep("first_intron|last_intron", DMR_bkgd_features$feature_simp),],aes(x = comparison, y = ..count.., fill = factor(feature_simp), group = feature_simp)) + geom_bar(position = "fill", color = "black") + scale_y_continuous(labels=scales::percent) + ylab("% of total regions") + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 60, hjust = 1), axis.title = element_text(size = 12, face = "bold")) + scale_fill_manual("Feature",values = RColorBrewer::brewer.pal(7,"BuGn")) + ggtitle("first and last introns percentages")

ggplot(DMR_bkgd_features_totals[grep("intron",DMR_bkgd_features_totals$feature_simp),],aes(x = factor(feature_simp, levels = c("intron", "first_intron", "last_intron")), y = count, fill = feature_simp)) + geom_bar(stat = "identity", color = "black") + ylab("count") + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 60, hjust = 1), axis.title = element_text(size = 12, face = "bold")) + ggtitle("first and last introns counts") + facet_wrap(~comparison, scale = "free") + scale_fill_manual("Feature",values = RColorBrewer::brewer.pal(7,"BuGn"))
...
DMR_feat_summary <- data.frame(table(DMR_bkgd_features[,c("feature","comparison")]))
#rename columns
...
#replace NAs with 0
#eat_summ[is.na(feat_summ)] <- 0
str(DMR_feat_summary[is.na(DMR_feat_summary)])

#create variables for the sum of all DMR and background features
feat_salDMR_total <- sum(DMR_feat_summary[which(DMR_feat_summary$comparison=="Sal"),"numRegion"])

feat_tempDMR_total <- sum(DMR_feat_summary[which(DMR_feat_summary$comparison=="Temp"),"numRegion"])

feat_tempxsalDMR_total <- sum(DMR_feat_summary[which(DMR_feat_summary$comparison=="TempxSal"),"numRegion"])

feat_tempANDsalDMR_total <- sum(DMR_feat_summary[which(DMR_feat_summary$comparison=="Temp+Sal"),"numRegion"])

feat_region_total <- sum(DMR_feat_summary[which(DMR_feat_summary$comparison=="background"),"numRegion"])

#create a table with Chi stats for each feature
chi_table <- data.frame() #create empty df
for(i in 1:length(unique(DMR_feat_summary$feature))){ #loop through each feature
  numBKGDReg <-DMR_feat_summary[which(DMR_feat_summary$comparison=="background"),"numRegion"][i] # variable for number of background regions for specific feature
  numsalDMRreg <- DMR_feat_summary[which(DMR_feat_summary$comparison=="Sal"),"numRegion"][i] # variable for number of DMRs for specific feature
  numtempDMRreg <- DMR_feat_summary[which(DMR_feat_summary$comparison=="Temp"),"numRegion"][i] # variable for number of DMRs for specific feature
  numtempxsalDMRreg <- DMR_feat_summary[which(DMR_feat_summary$comparison=="TempxSal"),"numRegion"][i] # variable for number of DMRs for specific feature
  numtempANDsalDMRreg <- DMR_feat_summary[which(DMR_feat_summary$comparison=="Temp+Sal"),"numRegion"][i] # variable for number of DMRs for specific feature
  ...
  x$feature <- as.character(DMR_feat_summary[which(DMR_feat_summary$comparison=="Sal"),"feature"][i]) # add feature name to chi sq df
  x$comparison <- "Sal"
  ...
  y$feature <- as.character(DMR_feat_summary[which(DMR_feat_summary$comparison=="Temp"),"feature"][i]) # add feature name to chi sq df
  y$comparison <- "Temp"
  ...
  z$feature <- as.character(DMR_feat_summary[which(DMR_feat_summary$comparison=="TempxSal"),"feature"][i]) # add feature name to chi sq df
  ...
  w$feature <- as.character(DMR_feat_summary[which(DMR_feat_summary$comparison=="Temp+Sal"),"feature"][i]) # add feature name to chi sq df
