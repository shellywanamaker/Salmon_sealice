...
for(i in 1:length(unique(feat_summ$feature))){ #loop through each feature
  ...
  print(ct_c)
  print(ct_d)
  x <-  data.frame(broom::tidy(prop.test(ct_a, correct = FALSE))) # create data frame storing the Chi sq stats results
  x$feature <- as.character(feat_summ[which(feat_summ$comparison=="Sal"),"feature"][i]) # add feature name to chi sq df
  x$comparison <- "Sal"
  y <-  data.frame(broom::tidy(prop.test(ct_b, correct = FALSE))) # create data frame storing the Chi sq stats results
  y$feature <- as.character(feat_summ[which(feat_summ$comparison=="Temp"),"feature"][i]) # add feature name to chi sq df
  y$comparison <- "Temp"
  z <-  data.frame(broom::tidy(prop.test(ct_c, correct = FALSE))) # create data frame storing the Chi sq stats results
  ...
  w <-  data.frame(broom::tidy(prop.test(ct_d, correct = FALSE))) # create data frame storing the Chi sq stats results
